...
| `dog_photos` | `id`, `dog_id` (FK), `storage_path`, `position` | Owner-only CRUD via `dogs.owner_id` join. Private Storage bucket, signed URLs only. Max 5 photos/dog, max 5MB/file. EXIF/GPS metadata stripped server-side (Vercel function using `sharp`) before the file reaches Storage. |
